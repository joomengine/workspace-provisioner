...
Restore is deliberately conservative: it recovers an **absent VM** into the retained workspace's existing host/project/address reservation. It does not overwrite an existing VM, allocate a new customer identity, reactivate a deleted workspace, or silently migrate between hosts. Restoring over an existing VM requires a separate explicit administrative retirement procedure; do not submit a normal provisioner `delete`, which retires the workspace itself.
